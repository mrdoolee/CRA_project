...
// The Express app + /api/* routes, shared between local/self-hosted use
// (server.ts, which adds static file serving and app.listen() on top of this)
// and the Vercel serverless entry (api/index.ts, which exports this app as-is
// with no listen() — Vercel invokes it per-request instead).
